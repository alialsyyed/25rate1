...
var MemStorage = class {
  feedbackResponses;
  constructor() {
    this.feedbackResponses = /* @__PURE__ */ new Map();
  }
...
async function registerRoutes(app2) {
  ...
  app2.get("/api/feedback", async (req, res) => {
    try {
      const feedbackResponses2 = await storage.getAllFeedbackResponses();
      res.json(feedbackResponses2);
    } catch (error) {
      res.status(500).json({ message: "Failed to retrieve feedback" });
    }
  });
  app2.get("/api/feedback/export", async (req, res) => {
    try {
      const feedbackResponses2 = await storage.getAllFeedbackResponses();
      const csvHeader = "ID,Rating,Source,Comments,Created At\n";
      const csvRows = feedbackResponses2.map((feedback) => {
